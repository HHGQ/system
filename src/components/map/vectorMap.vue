...
export default {
  ...
  methods: {
    initMap() {
      let vs = new ol.source.Vector({features: (new ol.format.GeoJSON({featureProjection: 'EPSG:4326'})).readFeatures(chinaGeo)})  //读取Geojson格式数据，只有通过这种方式才能出来！！！
      let zhanjiangVS = new ol.source.Vector({features: (new ol.format.GeoJSON({featureProjection: 'EPSG:4326'})).readFeatures(zhanjiang)})  //读取Geojson格式数据，只有通过这种方式才能出来！！！
      this.vs = vs
      console.log(vs)
      this.map = new ol.Map({
        target: 'map',
        layers: [
          new ol.layer.Tile({
            source: new ol.source.XYZ({
              url:
                "http://t4.tianditu.com/DataServer?T=vec_w&tk=5b1c7a4af317c1efaa03265e11a6204e&x={x}&y={y}&l={z}",
